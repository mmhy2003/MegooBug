...
The frontend uses a Cyberpunk-themed palette defined as **hex CSS custom
properties** in hand-written CSS at [`src/app/globals.css`](src/app/globals.css);
there is no Tailwind config. Two themes are declared as attribute selectors —
`[data-theme="dark"]` (also the `:root` default) and `[data-theme="light"]` — and
switched by [`next-themes`](src/components/theme-provider.tsx)
(`attribute="data-theme"`, `defaultTheme="dark"`), toggled in
[`src/components/theme-toggle.tsx`](src/components/theme-toggle.tsx).

- **Source of truth:** the hex values in `globals.css`. The HSL column below is a
  reference conversion.
- The token names in the tables below are **design names** (shadcn-style). In the
  CSS they are applied — by visual role, not by matching name — to the app's own
  variables: surfaces use `--bg-*` (e.g. `--bg-primary`, `--bg-card`), accents use
  `--accent-*` (`--accent-primary` / `-secondary` / `-error` / `-success` /
  `-warning`, each also exposed as an `…-rgb` triplet), and text uses `--text-*`.
  See `globals.css` for the exact per-variable assignment.
- `destructive` is reused for both `--accent-error` and the magenta
  `--accent-secondary` brand accent (used in gradients/glows).
- `success` and `warning` (`--accent-success` / `--accent-warning`) are custom
  tokens beyond the standard shadcn/ui set.
