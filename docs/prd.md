...
Stage 2: runner   → copy venv, run uvicorn
```

**`backend/Dockerfile.dev`** — single stage, mounts source, runs `uvicorn --reload`.
...
# ── Auth ──
ALLOW_SIGNUP=true                # true | false
JWT_ACCESS_TOKEN_EXPIRE_MINUTES=30
JWT_REFRESH_TOKEN_EXPIRE_DAYS=7
INVITE_TOKEN_EXPIRE_HOURS=48

# ── Database ──
DATABASE_URL=postgresql+asyncpg://megoo:password@postgres:5432/megoobug
POSTGRES_USER=megoo
...
|-------|-------|---------------|
| **Phase 1 — Foundation** | Project scaffold, Docker setup, Makefile, DB models, auth (login/signup/invite), user CRUD, role middleware | 2 weeks |
| **Phase 2 — Core** | Project CRUD, Sentry ingest endpoints, event processing & grouping, issue management | 2 weeks |
| **Phase 3 — Frontend** | Layout (sidebar, header), dashboard, project list/detail, issue detail with stack traces, user management page | 3 weeks |
